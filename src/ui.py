...
# 定义函数
def get_video_list_fast(time_to_sort, category, sort_by, max_num):
    '''
    :param time_to_sort: 1,2,3,4分别对应1小时，24小时，1周，1个月
    :param category: 1,2分别对应销量榜和热度榜
    :param sort_by: 传入所需选择的类目文本即可
    ...
    try:
        ...
        for percent in a.get_video_list_fast(time_to_sort, category, sort_by, max_num):
            p = percent / max_num
            progress.progress(p)  # 更新进度条
            percent_text.metric("进度", f"{p*100}%")  # 更新百分比数字
...
with st.sidebar:
    ...
    st.title('功能选择')
    # 功能选择栏
    function_options = ['视频榜单设置（快速版）', '视频榜单设置', '话题榜单设置']
...
if selected_function == '视频榜单设置（快速版）':
    with st.sidebar:
        st.title('视频榜单设置（快速版）')

        time_options = {1: '1小时', 2: '24小时', 3: '1周', 4: '1个月'}

        time_to_sort = st.selectbox('选择时间范围', list(time_options.values()), index=0, format_func=lambda x: x)
        # 类别选择栏
        category_options = {1: '销量榜', 2: '热度榜'}
        category = st.radio('选择类别', list(category_options.values()), format_func=lambda x: x)
    ...
    if 'df' not in st.session_state or 'params' not in st.session_state or st.session_state.params != params:
        if st.button('查询'):
            st.write('正在查询中！')
            st.session_state.df = get_video_list_fast(time_to_sort, category, sort_by, max_num)
            st.session_state.params = params
